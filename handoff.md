...
| # | Priority | Task |
|---|----------|------|
| 1 | BLOCKER | **Vercel cannot deploy: the Vercel GitHub App cannot see this repo.** Root cause found 2026-09-16 via the Vercel API, not a storage or quota problem (deleting old deployments did not help and was not needed). The installation for user `bugiiiii11` (id **161945200**) is set to "Only select repositories" and covers exactly three: `crm-core`, `mdn-tech`, `rein` -- which is why only those three projects still deploy. `zane_kadernictvo` is absent, so GitHub answers 404 on create-an-installation-access-token and every build dies before it starts (0 ms, "Git information retrieval failed", stuck on the old sha 16ae212). FIX: https://github.com/settings/installations/161945200 -> Repository access -> add `zane_kadernictvo` (or switch to All repositories), then Redeploy in Vercel. NOTE: ~18 other projects (swarm-resistance-frontend-dev, royal-stroje, ai-portal, ...) sit on the same stale credential and their repos are not in that list either -- they will fail the same way on their next deploy. |
| 2 | High | **After the deploy finally lands, verify the S12 work is live:** `curl -s https://ovlasy.sk | grep -c 'as="font"'` returns 6, `/favicon.ico` and `/site.webmanifest` return 200, the page shows +421 950 249 838, and the hero h1 no longer carries a zero-opacity inline style. |
| 3 | High | **Founder sign-off on copy** -- now also covers the SEO copy proposals in `SEO-AUDIT.md`: every heading is a manifesto line carrying no service or location (the lever is the first H2, not the H1), treatment durations appear nowhere, and the founder has no bio beyond a first name. |
| 4 | High | **Legal block incomplete.** Sec. 3a Obchodneho zakonnika wants the obchodne meno (e.g. "Zaneta <Priezvisko> - O VLASY") and the zivnostensky register entry (okresny urad + registration number). ICO/DIC are in already; add alongside in `Footer.tsx`. `llms.txt` deliberately says "Znacka" rather than asserting a legal name we do not have. |
| 5 | High | **SEO after migration:** GSC -- add the `ovlasy.sk` property (DNS TXT at Websupport), submit the sitemap, run Change of Address. GBP -- rename, new URL, new phone/email, and set Saturday as "by appointment" THERE (it is deliberately absent from the schema). IG/FB handles and bios still say `goodhairbyzane`. |
| 6 | Med | **Chatbot:** rewrite `chatbot-knowledge-base.md` + `knowledge-base.md` for the new brand/services (both still say Ivanka + extensions; only the contacts were updated in S12), upload to mdntech, then restore the `<Script>` from the comment in `src/app/layout.tsx`. |
| 7 | Med | **Mobile LCP will likely stay above 2.5 s** until the hero H1, rather than a 147 px sliver of the hero photo, becomes the LCP element. framer-motion is still the biggest main-thread blocker and `Reveal` keeps it on every section. A CSS/IntersectionObserver reveal would drop it for most of them. |
| 8 | Med | **Founder decision: old brand visible in two gallery photos.** `IMG_7166` has a "GOOD HAIR Club" mirror decal, `IMG_8938` a "HAIR CLUB" chair headrest. Ask whether to drop or crop. |
| 9 | Low | **Privacy page** -- the contact section embeds a Google Maps iframe that sets cookies with no consent and no policy page anywhere on the site. |
| 10 | Low | Run `/impeccable audit` + `/impeccable critique` (never formally audited). Delete 7 orphaned files in `public/produkty/` (`DSC_3617/3635/3649/3667/3691-HDR.jpg`, `DSC_3635-HDR.webp`, `DSC_3691-HDR.webp`) -- verify with `grep -rl` first. 8 gallery images still carry camera filenames. |
| 11 | Low | Real Google Reviews link once reviews exist (no schema rating at all now, by design). Online booking (Calendly/Booksy); MDX blog for "O vlasy s rozumom" only with a 6-month commitment. |
